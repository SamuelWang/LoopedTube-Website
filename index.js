function onPlayerReady() {
    loopedtube.player.addEventListener("onStateChange", function (a) {
        var c = a.data, b = $("#time-interval"); a = $("#starttime", b); b = $("#endtime", b); switch (c) {
            ...
            0 === e ? (loopedtube.currentVideo.duration = c, $("#endtime", "#time-interval").val($.secondToTime(c)), b.removeAttr("disabled")) : 0 !== e && b.removeAttr("disabled"), loopedtube.cued = !1, loopedtube.startWatchCurrentTime(), loopedtube.addRecentVideo(loopedtube.currentVideo, function () { loopedtube.renderRecentVedioList() }), $("#playin").val(loopedtube.currentVideo.id)); break; case YT.PlayerState.PAUSED: loopedtube.stopWatchCurrentTime(); break; case YT.PlayerState.CUED: d = loopedtube.cuedVideo.startTime ? loopedtube.cuedVideo.startTime :
            0, e = loopedtube.cuedVideo.endTime ? loopedtube.cuedVideo.endTime : 0, a.val($.secondToTime(d)), b.val($.secondToTime(e)), a.attr("disabled", "disabled"), b.attr("disabled", "disabled"), loopedtube.currentVideo = loopedtube.cuedVideo, loopedtube.cued = !0, loopedtube.player.playVideo()
        }
